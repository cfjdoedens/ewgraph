...
if (requireNamespace("devtools", quietly = TRUE) && 
    file.exists("ewgraph.Rproj")) {
  devtools::load_all(".")
} else {
  # Error! Break off knitting process.
    stop("README ewgraph: no ewgraph.Rproj in current directory.")
}
...
[![Codecov test coverage](https://codecov.io/gh/cfjdoedens/ewgraph/graph/badge.svg)](https://app.codecov.io/gh/cfjdoedens/ewgraph)
<!-- badges: end -->

The goal of ewgraph is to represent and manipulate chance graphs as a
finite grid of data elements. The individual elements are called segments.
The grid is formed as a tibble. Each segment is a row of the tibble.

The package design is not based on information hiding: the idea is
that users can know the details of how the chance graph is represented. And
...
$c$ is interpreted in this context as a function that maps from an
...
This function, $c$, is represented in an ewgraph as a finite set of $S$ pairs
$(p_i, h_i)$, $i$ in $1:S$. $S$ a positive integer.
$p_i$ are elements of P, so of [0, 1].
$h_i$ are elements of H, so of [0, inf].
$$h_i = c(p_i)$$
...
The middle of the ith segment being $$p_i = i /(S - U)$$
...
We approximate the surface of the probability function
...
```{r first approximation of segment surface}
#                  |hi
#                  |
#                  |
#                  |
...
the segment. So by computing $h_{p_{i-U}} \equiv hleft_i$, and
$h_{p_{i+U}} \equiv hright_i$.
We can then compute the surface of segment $i$ as the sum
of its left and right parts.

```{r second approximation of segment surface}
#                  |hi
#                  |
#                  |               |hrighti
#  |hlefti         |               |
...
```
 ...
 For $hleft_i$ we proceed:
 Let 
 $$h_{im1} = c(p_{im1})$$ 
 where $im1$ stands for $i - 1$.
 We then interpolate:
 $$hleft_i  = (h_i + h_{im1}) / 2$$
```{r interpolation of hlefti}
#                                                  |hi
#                                  |hlefti         |
#                  |him1           |               |
#                  |               |               |
...
```
  ...
  $$hright_i = (h_i + h_{ip1}) / 2$$
```{r interpolation of hrighti}
#                  |hi
#                  |               |hrighti
#                  | hi            |               |hip1
#                  |               |               |
...
```
  ...
  This leaves us with the problem how to interpolate the leftmost $hleft_i$, so $hleft_1$,
  and the rightmost $hright_i$, so $hright_S$.
...
```
  ...
  The line through the coordinates $(U, h_1)$ and $(3 * U, h_2)$
  ...
  Then we get
    $$a = (h_2 - h_1) / (2 * U)$$
    $$b = (3 * h_1 - h_2) / 2$$
  So for $hleft_1$ we get
    $$hleft_1 = a * 0 + b$$
           $$= b$$
           $$= (3 * h_1 - h_2) / 2$$
  However, note that $hleft_1$ could become less than 0.
...
```
  ...
  As with $hleft_1$ we should forbid negative values for $hright_S$.
  So, taking this into account we get:
...
A segment of an ewgraph ABDEF can be considered to consist of two rectangular
...
each length U.
Line AB has length $hleft_i$.
Line ED has length $hright_i$.
```{r representation of segment from as a geometric figure}
#                  F
#                  "
#                 / \
#                / | \
...
and the segments of the ewgraph.

In contrast to the cited method an ewgraph
...
height at $p = 0$ or at $p = 1$. By steering clear from these
...
In the programming language R we represent the ewgraph as a tibble with S rows.
Each ith segment of the ewgraph has the following variables:
