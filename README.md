...
algebraic form $y = ax+b$. So $$h_1 = a * U + b$$
$$h_2 = a * 3 * U + b$$ Then we get $$a = (h_2 - h_1) / (2 * U)$$
$$b = (3 * h_1 - h_2) / 2$$ So for $hleft_1$ we get
$$hleft_1 = a * 0 + b$$ $$= b$$ $$= (3 * h_1 - h_2) / 2$$ However, note
that $hleft_1$ could become less than 0. This we should forbid, as it
would imply a negative probability density.
...
The line through the coordinates $(1 - 3 * U, h_{Sm1})$ and
$(1 - U, h_S)$ has the algebraic form $$y = ax+b$$ So
$$h_{Sm1} = a * (1 - 3 * U) + b$$ $$h_S   = a * (1 - U) + b$$ Then we
get $$a = (h_S - h_{Sm1}) / (2 * U)$$
